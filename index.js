...
function getCurrentWeek() {
  ...
  const year = now.getFullYear();
  const seasonStarts = [
    new Date(year, 0, 7),   // S1: Jan 7
    new Date(year, 3, 7),   // S2: Apr 7
    new Date(year, 6, 7),   // S3: Jul 7
    new Date(year, 9, 7),   // S4: Oct 7
  ];

  let seasonIdx = 0;
  for (let i = 0; i < seasonStarts.length; i++) {
    if (now >= seasonStarts[i]) seasonIdx = i;
    else break;
  }

  const start = seasonStarts[seasonIdx];
  const diffMs = now - start;
  const diffDays = Math.floor(diffMs / (1000 * 60 * 60 * 24));
  const week = Math.floor(diffDays / 7) + 1; // week 1 = Jan 7–13, etc.

  return { week, season: seasonIdx + 1 };
}
// ====================== STORAGE ======================
...
function buildLeaderboardHTML(drivers, seasonLabel) {
  const { week } = getCurrentWeek();
  const rows = drivers.slice(0, 15).map((d, i) => {
    const rank = i + 1;
    const medal = rank === 1 ? "🥇" : rank === 2 ? "🥈" : rank === 3 ? "🥉" : rank;
    const change = d.lastChange === undefined ? ""
      : d.lastChange > 0 ? `▲ +${d.lastChange}`
      : d.lastChange < 0 ? `▼ ${d.lastChange}`
      : "—";
    const changeClass = d.lastChange > 0 ? "change-up" : d.lastChange < 0 ? "change-down" : "change-zero";

    // Progress bar width (fake percentile feel – top ≈90%, bottom ≈30%)
    const progressWidth = Math.max(30, 90 - (rank - 1) * 4) + "%";

    let barColor = "var(--lime)";
    if (rank === 1) barColor = "#ffd700";
    else if (rank === 2) barColor = "#c0c0c0";
    else if (rank === 3) barColor = "#cd7f32";

    const rowBg = rank <= 3 ? "rgba(168,208,0,0.08)" : "transparent";

    return `
      <div class="row" style="background:${rowBg};">
        <div class="rank">${medal}</div>
        <div class="name">${d.iracingName || "—"}</div>
        <div class="irating">${d.lastIRating ?? "?"}</div>
        <div class="change ${changeClass}">${change}</div>
        <div class="progress">
          <div class="bar" style="width:${progressWidth}; background:${barColor};"></div>
        </div>
      </div>
    `;
  }).join("");
...
<meta charset="UTF-8">
<style>
  @import url('https://fonts.googleapis.com/css2?family=Barlow+Condensed:wght@400;600;700;900&family=Chakra+Petch:wght@400;600;700&display=swap');
  :root {
    --lime: #a8d000;
    --lime-glow: #c2f000;
    --text: #ffffff;
  ...
  body {
    margin:0; padding:0; background:#060606; color:var(--text);
    font-family: 'Chakra Petch', monospace;
  ...
  .card {
    ...
    border:3px solid var(--lime);
    box-shadow: 0 0 0 1px #000, 0 0 30px rgba(168,208,0,0.6), inset 0 1px 0 rgba(255,255,255,0.05);
    position:relative; background:var(--dark);
  }
  .carbon-bg {
    position:absolute; inset:0;
    background:url('data:image/png;base64,${B64_CARBON}') center/cover;
    opacity:0.7;
  }
  .content { position:relative; z-index:2; padding:24px 28px; }
  h1 {
    font-family: 'Barlow Condensed', sans-serif;
    font-size:52px; font-weight:900; text-align:center;
    letter-spacing:0.04em; text-transform:uppercase;
    background:linear-gradient(90deg, #fff, var(--lime-glow), #fff);
    -webkit-background-clip:text; -webkit-text-fill-color:transparent;
    margin:0 0 4px;
    text-shadow:0 0 30px rgba(168,208,0,0.5);
  }
  .subtitle {
    text-align:center; font-size:16px; font-weight:600;
    color:var(--muted); letter-spacing:0.12em; margin-bottom:28px;
  }
  .header-line {
  ...
  .row {
    ...
    border-radius:6px; margin-bottom:4px; font-size:15px;
  }
  .rank { width:50px; font-weight:900; font-size:18px; text-align:center; }
  .name { flex:1; font-weight:600; letter-spacing:0.03em; }
  .irating { width:110px; text-align:right; font-weight:700; font-size:18px; }
  .change { width:90px; text-align:right; font-weight:700; }
  .change-up { color:#4ade80; }
  .change-down { color:#f87171; }
  .change-zero { color:#aaa; }
  .progress { width:100%; height:3px; background:rgba(255,255,255,0.08); margin-top:6px; border-radius:2px; overflow:hidden; }
  .bar { height:100%; transition:width 0.6s ease; }
  .footer {
    margin-top:28px; padding-top:16px; border-top:1px solid rgba(168,208,0,0.18);
...
<div class="card">
  ...
  <div class="content">
    ...
    <div class="footer">
      <span>Gamma Sim Racing · iRacing Data</span>
      <span>${seasonLabel} · Week ${week}</span>
    </div>
  </div>
</div>
</body>
...
async function renderLeaderboardCard(drivers) {
  const { label: seasonLabel } = getCurrentSeason();
  const html = buildLeaderboardHTML(drivers, seasonLabel);
  ...
  try {
    const page = await browser.newPage();
    await page.setViewport({ width: 520, height: 1200 }); // generous height
    await page.setContent(html, { waitUntil: "networkidle0" });
    const height = await page.evaluate(() => document.body.scrollHeight);
    await page.setViewport({ width: 520, height });
    return await page.screenshot({ type: "png" });
  } finally {
    await browser.close();
  }
  ...
  try {
    ...
    if (drivers.length === 0) {
      return isInteraction
        ? interactionOrChannel.editReply({ content: "No drivers linked yet." })
        : interactionOrChannel.send({ content: "No drivers linked yet." });
    }

    // Update current iRatings (but only save if it's the weekly cron)
    for (const d of drivers) {
      const ir = await getCurrentIRating(d).catch(() => null);
      if (ir !== null) {
        const old = d.lastIRating ?? ir;
        d.lastIRating = ir;
        d.lastChange = ir - old;
      }
    }
    ...
    if (saveBaseline) {
      saveLinkedDrivers(drivers);
      console.log("Weekly iRating baseline saved.");
    }

    const buffer = await renderLeaderboardCard(drivers);

    const msg = {
      content: drivers.length > 15 ? `Top 15 of ${drivers.length} linked drivers shown.` : null,
      files: [new AttachmentBuilder(buffer, { name: "gsr-leaderboard.png" })]
    };

    if (isInteraction) await interactionOrChannel.editReply(msg);
    else await interactionOrChannel.send(msg);

  } catch (err) {
    console.error("Leaderboard render error:", err);
    const errMsg = { content: "❌ Failed to generate leaderboard image." };
    if (isInteraction) await interactionOrChannel.editReply(errMsg);
    else await interactionOrChannel.send(errMsg);
  }
}
// ====================== REGISTER COMMANDS ======================
const commands = [
